...
def count_flips_to(teststr):
    flips = ""
    possibilities = ["H", "T"]
    while not flips.endswith(teststr):
        flips += random.choice(possibilities)
    return len(flips)


def empirical_grind(teststr):
    data = 0
    for i in range(100000):
        data += (count_flips_to(teststr) / 100000)
    return data
...
def main():
    ...
    empirical_data = empirical_grind(teststr)
    print("The expected number of coin flips to get your string is as follows:")
    print(answer[0][0])
    print("Empirically, we see that the expected value should be roughly")
